...
import time
    ...
    def __init__(self, initial=[1, 3, 5, 7]):
        ...
        self.player = Nim.INITIAL_PLAYER
        ...
...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(state):
            args = (tuple(state), tuple(action))
            if args not in self.q:
                self.q[args] = 0
        return max(self.q[(tuple(state), tuple(action))] for action in Nim.available_actions(state))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if self.epsilon == False:
            return max(Nim.available_actions(state), key=lambda action: self.get_q_value(state, action))
        else:
